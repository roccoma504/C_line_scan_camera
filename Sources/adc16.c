 ...
 * c) Call the ADC_Config_Alt function again to restore desired configuration
 *    after a calibration
 *
 *
 */
...
void InitADCs()
{
    ...
    // Master_Adc_Config.STATUS1A = AIEN_ON | DIFF_SINGLE | ADC_SC1_ADCH(25);
...
void ADC1_IRQ()
{
	unsigned char junk =  ADC1_RA;
}


//Here are 2 functions that will read an ADC Channel via "Polling"
//It assumes the InitADCs() function has been called with 8-bit mode enabled.
//It also assumes The interrupts are NOT enabled.
unsigned char Read_ADC0(unsigned char Channel)
{
...
unsigned char Read_ADC1(unsigned char Channel)
{
	ADC1_SC1A  =  Channel;
